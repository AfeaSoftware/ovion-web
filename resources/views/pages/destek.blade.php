...
@php
  ...
  $quickActions = collect($content?->get('quick_actions') ?? []);
@endphp
    ...
    <div class="sp-actions-grid">
      @foreach($quickActions as $i => $action)
        @php
          ...
          $iconKey = $action['icon'] ?? 'shield';
        @endphp
        <a href="{{ $action['url'] ?? '' }}" class="sp-action sp-reveal{{ $delayClass }}">
          ...
          <p>{{ $action['desc'] ?? '' }}</p>
          @if(! empty($action['cta']))
            <div class="sp-action-arrow">{{ $action['cta'] }} <svg width="11" height="11" viewBox="0 0 12 12" fill="none"><path d="M2 6h8M6 2l4 4-4 4" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg></div>
          @endif
        </a>
      @endforeach
    </div>
